...
namespace internal
{
   ...
   */
    class MessageHandler
    {
        ...
        inline MessageHandler& operator<<(std::ostream& (*basic_manipulator)(std::ostream&))
        {
...
#define AP_ASSERT(...) SELECT_MACRO(__VA_ARGS__, _AP_ASSERT2, _AP_ASSERT1, dummy)(__VA_ARGS__)
...
#define _AP_ASSERT2(cond, extype) _AP_ASSERT_(cond, extype) << "  condition: " << #cond << " is not true\n\n"
...
#define AP_ASSERT_IN_RANGE(position, lower_bound, upper_bound)         \
    _AP_ASSERT((position >= lower_bound) && (position <= upper_bound)) \
      << "Out of range: " << position << " is not in range [" << lower_bound << ", " << upper_bound << "]\n\n"

#define AP_ASSERT_EQ(a, b) _AP_ASSERT((a == b)) << a << " was expected to be equal to " << b << std::endl

#define AP_ASSERT_NE(a, b) _AP_ASSERT((a != b)) << a << " was expected to be not equal to " << b << std::endl

#define AP_ASSERT_LT(a, b) _AP_ASSERT((a < b)) << a << " was expected to be less than " << b << std::endl

#define AP_ASSERT_LE(a, b) _AP_ASSERT((a <= b)) << a << " was expected to be less or equal than " << b << std::endl

#define AP_ASSERT_GT(a, b) _AP_ASSERT((a > b)) << a << " was expected to be greater than " << b << std::endl

#define AP_ASSERT_GE(a, b) _AP_ASSERT((a >= b)) << a << " was expected to be greater or equal than " << b << std::endl

#define AP_ERROR_IN_RANGE(position, lower_bound, upper_bound)        \
    AP_ERROR((position >= lower_bound) && (position <= upper_bound)) \
      << "Out of range: " << position << " is not in range [" << lower_bound << ", " << upper_bound << "]\n\n"

#define AP_ERROR_EQ(a, b) AP_ERROR((a == b)) << a << " was expected to be equal to " << b << std::endl

#define AP_ERROR_NE(a, b) AP_ERROR((a != b)) << a << " was expected to be not equal to " << b << std::endl

#define AP_ERROR_LT(a, b) AP_ERROR((a < b)) << a << " was expected to be less than " << b << std::endl

#define AP_ERROR_LE(a, b) AP_ERROR((a <= b)) << a << " was expected to be less or equal than " << b << std::endl

#define AP_ERROR_GT(a, b) AP_ERROR((a > b)) << a << " was expected to be greater than " << b << std::endl

#define AP_ERROR_GE(a, b) AP_ERROR((a >= b)) << a << " was expected to be greater or equal than " << b << std::endl
