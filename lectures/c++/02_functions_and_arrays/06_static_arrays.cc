...
int main()
{
    ...
    for (std::size_t k{0}; k < 4; ++k) std::cout << "ai[" << k << "] = " << ai[k] << '\n';

    std::cout << "\n-------------------------------\n\n";

    for (std::size_t k{0}; k < 9; ++k) std::cout << "af[" << k << "] = " << af[k] << '\n';
    ...
    *ad = 9.9;  // an array is implicitly converted to
    ...
    for (std::size_t k{0}; k < 7; ++k) std::cout << ad[k] << std::endl;
